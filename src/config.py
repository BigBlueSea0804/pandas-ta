...
#
# pivot_period는 피봇 계산에 쓸 EOD 일봉을 얼마나 받을지다. TV의 "Use daily-based
# values"(기본 켜짐)가 인트라데이 차트에서도 인트라데이 봉을 합치지 않고 EOD 일봉을
# 따로 요청하기 때문이다(같은 날이라도 두 값은 어긋난다). None이면 이미 받아 둔 봉이
# EOD 일봉이라 그대로 쓴다.
...
TIMEFRAMES: tuple[Timeframe, ...] = (
    Timeframe("1분", "1m", "5d", PIVOT_ANCHOR_DAILY, pivot_period="1mo"),
    Timeframe("5분", "5m", "1mo", PIVOT_ANCHOR_DAILY, pivot_period="1mo"),
    Timeframe("15분", "15m", "1mo", PIVOT_ANCHOR_DAILY, pivot_period="1mo"),
    Timeframe("30분", "30m", "1mo", PIVOT_ANCHOR_WEEKLY, pivot_period="3mo"),
    Timeframe("1시간", "60m", "2y", PIVOT_ANCHOR_WEEKLY, pivot_period="3mo"),
    Timeframe("2시간", "60m", "2y", PIVOT_ANCHOR_WEEKLY, pivot_period="3mo", resample_rule="2h"),
    Timeframe("4시간", "60m", "2y", PIVOT_ANCHOR_WEEKLY, pivot_period="3mo", resample_rule="4h"),
    Timeframe("1일", "1d", DEFAULT_PERIOD, PIVOT_ANCHOR_MONTHLY),
    Timeframe("1주", "1wk", "5y", PIVOT_ANCHOR_YEARLY, pivot_period="5y"),
    Timeframe("1달", "1mo", "10y", PIVOT_ANCHOR_YEARLY, pivot_period="5y"),
)
