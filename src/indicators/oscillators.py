...
from config import BBP_EMA_LENGTH, TREND_EMA_LENGTH
...
def compute_oscillators(ohlcv: pd.DataFrame) -> list[Signal]:
    ...
    rsi, prev_rsi = last_two(ta.rsi(close, length=14))
    ...
    # TV의 priceAvg: 스토캐스틱 RSI·BBP의 추세 필터는 EMA50을 쓴다.
    trend_ema = last_number(ta.ema(close, length=TREND_EMA_LENGTH))
    ...
    return [
        Signal(id="rsi_14", name="상대 강도 지수 (14)", value=rsi, action=action_rsi(rsi, prev_rsi)),
        Signal(
            id="stoch_k",
            name="스토캐스틱 %K (14, 3, 3)",
            value=stoch_k,
        ...
        Signal(
            ...
            value=stochrsi_k,
            action=action_stochrsi(stochrsi_k, stochrsi_d, last_close, trend_ema),
        ),
        Signal(
            id="willr_14",
        ...
        Signal(
            ...
            name="불 베어 파워",
            value=bbp,
            action=action_bbp(last_close, trend_ema, bull_now, bear_now, bull_prev, bear_prev),
        ),
        Signal(
            id="uo",
